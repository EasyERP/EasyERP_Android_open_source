...
public class SlidingPaneMenuDragHelper {
    private static final String TAG = "SlidingPaneMenuDragHelper";
    ...
    private final SlidingPaneMenuDragHelper.Callback mCallback;
    ...
    /**
     * A Callback is used as a communication channel with the SlidingPaneMenuDragHelper back to the
     * parent view using it. <code>on*</code>methods are invoked on siginficant events and several
     * accessor methods are expected to provide the SlidingPaneMenuDragHelper with more information
    ...
    public abstract static class Callback {
         ...
         * one of these methods, the SlidingPaneMenuDragHelper will enter {@link #STATE_SETTLING}
         * and the view capture will not fully end until it comes to a complete stop.
         * If neither of these methods is invoked before <code>onViewReleased</code> returns,
         * the view will stop in place and the SlidingPaneMenuDragHelper will return to
         * {@link #STATE_IDLE}.</p>
         *
         ...
         *
         * <p>SlidingPaneMenuDragHelper may call this method multiple times for the same view even if
    ...
    /**
     * Factory method to create a new SlidingPaneMenuDragHelper.
     *
     * @param forParent Parent view to monitor
     * @param cb Callback to provide information and receive events
     * @return a new SlidingPaneMenuDragHelper instance
     */
    public static SlidingPaneMenuDragHelper create(ViewGroup forParent, SlidingPaneMenuDragHelper.Callback cb) {
        return new SlidingPaneMenuDragHelper(forParent.getContext(), forParent, cb);
    }
     ...
     * @return a new SlidingPaneMenuDragHelper instance
     */
    public static SlidingPaneMenuDragHelper create(ViewGroup forParent, float sensitivity, SlidingPaneMenuDragHelper.Callback cb) {
        final SlidingPaneMenuDragHelper helper = create(forParent, cb);
        helper.mTouchSlop = (int) (helper.mTouchSlop * (1 / sensitivity));
        return helper;
    }

    /**
     * Apps should use SlidingPaneMenuDragHelper.create() to get a new instance.
     ...
     * @param forParent Parent view to monitor
     */
    private SlidingPaneMenuDragHelper(Context context, ViewGroup forParent, SlidingPaneMenuDragHelper.Callback cb) {
        if (forParent == null) {
     ...
     * The callback's {@link SlidingPaneMenuDragHelper.Callback#onEdgeTouched(int, int)} and
     * {@link SlidingPaneMenuDragHelper.Callback#onEdgeDragStarted(int, int)} methods will only be invoked
     ...
     * but {@link SlidingPaneMenuDragHelper.Callback#tryCaptureView(android.view.View, int)} will not be asked permission to
     * capture this view.
     *
    ...
    public void captureChildView(View childView, int activePointerId) {
        if (childView.getParent() != mParentView) {
            throw new IllegalArgumentException("captureChildView: parameter must be a descendant "
                    + "of the SlidingPaneMenuDragHelper's tracked parent view (" + mParentView + ")");
        }
     ...
     * of the SlidingPaneMenuDragHelper's knowledge).
     *
    ...
    public boolean shouldInterceptTouchEvent(MotionEvent ev) {
        ...
        switch (action) {
            ...
            case MotionEventCompat.ACTION_POINTER_DOWN: {
                ...
                // A SlidingPaneMenuDragHelper can only manipulate one view at a time.
                if (mDragState == STATE_IDLE) {
                    final int edgesTouched = mInitialEdgesTouched[pointerId];
                    if ((edgesTouched & mTrackingEdges) != 0) {
                        mCallback.onEdgeTouched(edgesTouched & mTrackingEdges, pointerId);
     ...
     * The child order is determined using {@link SlidingPaneMenuDragHelper.Callback#getOrderedChildIndex(int)}.
     *
     * @param x X position to test in the parent's coordinate system
     * @param y Y position to test in the parent's coordinate system
    ...
    public float getInitialMotionXForPointer(int pointerId) {
        return mInitialMotionX[pointerId];
    }
}
